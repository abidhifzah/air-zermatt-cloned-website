! function(t, e) {
    ...
    m.prototype.init = function() {
        ...
        var e, o, n, s, d, l, a = (e = function(e) {
            ...
            d = this, s = [].slice.call(arguments, 0), l = 0;
            var t = function() {
                var i = new Date - l;
...
if (!document.getElementById("EmbedSocialIFrameLightboxCSS")) {
    ...
    cssEmbed.id = "EmbedSocialIFrameLightboxCSS", cssEmbed.rel = "stylesheet", cssEmbed.href = "https://embedsocial.com/cdn/iframe-lightbox.min.css?v=2.2";
...
EMBEDSOCIALHASHTAG = {
        ...
        if (document.getElementById("EmbedSocialIFrame") && "function" === typeof iFrameResize) {
            iFrameResize({
                messageCallback: function(messageData) {
                    ...
                    if (messageData.message.action === "appendGetFromParentPageWidthAttribute") {
                        EMBEDSOCIALHASHTAG.appendGetFromParentPageWidthAttribute(messageData.message)
                    }
        ...
        if (data.version === "free") {
            ...
            if (iframe) {
                ...
                if (freeBanners.length === 0) {
                    var freeLinkArrow = data.removeBrandingArrow ? "" : "<span> &#8594;</span>";
                    ...
                    freeLinkElement.innerHTML = (data.freeLinkText ? data.freeLinkText : "Widget by EmbedSocial") + freeLinkArrow;
    ...
    appendGetFromParentPageWidthAttribute: function(data) {
        var iframe = document.getElementById(data.iframeId);
        if (!iframe) {
            return
        }
        var parentElement = iframe.parentElement;
        if (!parentElement || !parentElement.classList.contains("embedsocial-hashtag")) {
            return
        }
        parentElement.setAttribute("data-send-page-width", "true");
        EMBEDSOCIALHASHTAG.sendParentPageWidth(iframe, window.innerWidth)
    },
    sendParentPageWidth: function(iframe, width) {
        if (typeof width !== "number" || !iframe || !iframe.iFrameResizer || !iframe.iFrameResizer.sendMessage) {
            return
        }
        iframe.iFrameResizer.sendMessage({
            parentPageWidth: width
        })
    },
    observePageWidth: function() {
        var debounce = null;
        window.addEventListener("resize", function() {
            ...
            debounce = setTimeout(function() {
                var widgets = document.querySelectorAll('.embedsocial-hashtag[data-send-page-width="true"]');
                for (var i = 0; i < widgets.length; i++) {
                    var iframes = widgets[i].getElementsByTagName("iframe");
                    if (!iframes || !iframes.length || !iframes[0]) {
                        continue
                    }
...
EMBEDSOCIALHASHTAG.observePageWidth();